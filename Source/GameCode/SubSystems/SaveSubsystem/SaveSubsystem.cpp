...
void USaveSubsystem::LoadLastGame()
{
	...
	if (SaveIds.Num() == 0)
	{
		UE_LOG(LogSaveSubsystem, Display, TEXT("USaveSubsystem::LoadLastGame(): %s Failed! No saves."),
		       *GetNameSafe(this));
		return;
	}
...
{
	UE_LOG(LogSaveSubsystem, Display, TEXT("USaveSubsystem::SerializeLevel(): %s, Level: %s, StreamingLevel: %s"),
	       *GetNameSafe(this), *GetNameSafe(Level), *GetNameSafe(StreamingLevel));
...
{
	UE_LOG(LogSaveSubsystem, Display, TEXT("USaveSubsystem::DeserializeLevel(): %s, Level: %s, StreamingLevel: %s"),
	       *GetNameSafe(this), *GetNameSafe(Level), *GetNameSafe(StreamingLevel));
	...
	for (TArray<AActor*>::TIterator ActorIterator = Level->Actors.CreateIterator(); ActorIterator; ++ActorIterator)
	{
		...
		for (TArray<FActorSaveData*>::TIterator ActorSaveDataIterator = ActorsSaveData.CreateIterator();
		     ActorSaveDataIterator; ++ActorSaveDataIterator)
		{
		...
		{
			UE_LOG(LogSaveSubsystem, Display,
			       TEXT("USaveSubsystem::DeserializeLevel(): %s, ActorSaveData not found! Destroy actor: %s"),
	...
	{
		UE_LOG(LogSaveSubsystem, Display, TEXT("USaveSubsystem::DeserializeLevel(): %s, Spawn new actor with name: %s"),
		       *GetNameSafe(this), *ActorSaveData->Name.ToString());
		...
		if (!IsValid(Actor))
		{
			...
			UE_LOG(LogSaveSubsystem, Display,
			       TEXT("USaveSubsystem::DeserializeLevel(): %s, Failed to spawn new actor with name: %s"),
			       *GetNameSafe(this), *ActorSaveData->Name.ToString());
			continue;
		}
...
{
	UE_LOG(LogSaveSubsystem, Display, TEXT("USaveSubsystem::LoadSaveFromFile(): %s, SaveId %i"), *GetNameSafe(this),
	       SaveId);
...
{
	UE_LOG(LogSaveSubsystem, Display, TEXT("USaveSubsystem::OnPostLoadMapWithWorld(): %s, World: %s"),
	       *GetNameSafe(this), *GetNameSafe(LoadedWorld));
	DeserializeGame();
}

void USaveSubsystem::DeserializeActor(AActor* Actor, const FActorSaveData* ActorSaveData)
{
	UE_LOG(LogSaveSubsystem, Display, TEXT("USaveSubsystem::DeserializeActor(): %s, Actor %s"), *GetNameSafe(this),
	       *GetNameSafe(Actor));
	...
	for (UActorComponent* ActorComponent : Actor->GetComponents())
	{
		if (ActorComponent->Implements<USaveSubsystemInterface>())
		{
			const FObjectSaveData* ComponentSaveData = ComponentsSaveData.FindByPredicate(
				[=](const FObjectSaveData& SaveData) { return SaveData.Name == ActorComponent->GetFName(); });
...
{
	UE_LOG(LogSaveSubsystem, Display, TEXT("USaveSubsystem::OnActorSpawned(): %s, Actor %s"), *GetNameSafe(this),
	       *GetNameSafe(SpawnedActor));
	if (bIgnoreOnActorSpawnedCallback)
	{
		UE_LOG(LogSaveSubsystem, Display,
		       TEXT("USaveSubsystem::OnActorSpawned(): %s, Skipped for actor %s. bIgnoreOnActorSpawnedCallback == true!"
		       ), *GetNameSafe(this), *GetNameSafe(SpawnedActor));
		return;
	}
